...
# add dual attention moduel
from attention import DAHead


# --- Downsampling block in TNet  --- #
...
class TNet(nn.Module):
    def __init__(self, height=4, width=6, depth_rate=16, num_dense_layer=4, growth_rate=16, in_channels=6, out_channels=3, kernel_size=3, stride=2, attention=True):
        super(TNet, self).__init__()
        self.rdb_module = nn.ModuleDict()
        self.upsample_module = nn.ModuleDict()
        self.downsample_module = nn.ModuleDict()
        self.Da_module = nn.ModuleDict()
        self.height = height
        self.width = width
        self.stride = stride
        ...
        # RDB module and dual attention module
        ...
        for i in range(height):
            if (i == 0):
                ...
                j = width // 2 - 1
                if (i == height - 1):
                    self.Da_module.update({'{}_{}'.format(i, j): DAHead(rdb_in_channels)})
                else:
                    self.rdb_module.update({'{}_{}'.format(i, j): RDB(rdb_in_channels, num_dense_layer, growth_rate)})
    ...
    def forward(self, x):
        ...
        for i in range(1, self.height):
            ...
            x_index[i][j] = self.downsample_module['{}_{}'.format(i-1, j)](x_index[i-1][j])


        x_index[i][j+1] = self.Da_module['{}_{}'.format(i, j)](x_index[i][j])
        k = j
